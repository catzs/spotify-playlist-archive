    ...
    for tup in playlist_nammes:
        ...
        try:
            ...
            if len(tup)>2 and not tup[2]==playlis[1]: #if there exists a name that differs from the one that spotify generated, then change the name element in playlist to the custom playlist name
                playlist=Playlist(url=playlis[0], name=tup[2].strip(), description=playlis[2], tracks=playlis[3]) #create modified tuple if there is a custom name
            else:
            ...
            for path, func, flag, flag2 in [ #adds check to preserve the custom text so it won't always get overwritten
                ...
                if flag:
                    ...
                    args = [playlist_id, Playlist(url=playlis[0], name=("custom:"+tup[2]).strip(), description=playlis[2], tracks=playlis[3])] #sends a modified playlist tuple that includes "custom" at the beginning
                else:
